...
- View 화면 호출 과정
  ...
  UIKit -> UIWindiw -> RootViewController -> ContentsViewControllers(TableViewViewController, NavigationViewController, .....)
...
- ViewDidAppear는 누가 호출하는가?
  ...
  코드상에서 view를 그린다면 loadView의 기본 구현이 오버헤드가 된다...? 따라서 오버라이딩해서 view를 구현하지 않는다.
